...
export class Admin implements OnInit {
  ...
  protected updateStatus(appointment: Appointment, status: AppointmentStatus): void {
    ...
    this.adminApi.updateAppointmentStatus(appointment.id, status).pipe(finalize(() => this.updatingId.set(null))).subscribe({ next: (updated) => this.appointments.update((items) => items.map((item) => item.id === updated.id ? updated : item)), error: (error: unknown) => this.errorMessage.set(getApiErrorDetails(error).message) });
  }
